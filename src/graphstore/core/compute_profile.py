...
def _base_profile(name: str, physical_cores: int) -> tuple[int, int, int, int, bool]:
    ...
    if name == "laptop":
        return (2, min(3, physical_cores), min(3, physical_cores), 32, False)
    if name == "desktop":
        return (2, min(6, physical_cores), min(6, physical_cores), 64, True)
    if name == "gpu":
        return (2, min(8, max(4, physical_cores)), min(8, max(4, physical_cores)), 128, True)
...
def describe_profile() -> str:
    ...
    return (
        f"profile={p.name} cores={p.cores}/{p.logical_cores} ram={ram}{gpu}{bat} "
