---
title: Edge matrix internals
sidebar_position: 3
---

# Edge matrix internals
...
| `get_combined()` | Union of all types `sum A_k` | Cross-type traversal, global degree |
| `get_combined_transpose()` | Union of transposes `sum A_k.T` | Cross-type incoming queries |
| `get_combined_spread()` | Symmetric `sum (A_k + A_k.T)` | Spreading activation, RECALL, HybridRAG |
...
| `_combined_all` | N/A | Frozen union of all types |
| `_transpose_cache` | edge type string | Frozen per-type transpose CSR |
| `_combined_transpose` | N/A | Frozen union of all transposes |
| `_combined_spread` | N/A | Frozen symmetric closure |
...
Used by the graph signal in REMEMBER fusion (signal 4: normalised degree).
