...
**tinygraph** is a lightweight, in-memory graph database built on scipy sparse matrices and numpy arrays. It ships with a custom DSL designed for readability — no Cypher, no SPARQL, just plain English-like queries.
...
```bash
git clone https://github.com/orkait/tinygraph.git
cd tinygraph
