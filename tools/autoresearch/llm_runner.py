...
Usage (benchmarks):
    ...
    from tools.autoresearch.llm_runner import LLMRunner, get_shared_runner
...
def get_shared_runner() -> "LLMRunner":
    ...
    if _SHARED is None:
        from tools.autoresearch.providers import load_config, resolve_providers
        from benchmarks.framework.transport.llm_client import QA_MODEL, QA_MODEL_OR
        providers = resolve_providers(load_config(), model_priority=[QA_MODEL, QA_MODEL_OR])
        if not providers:
            raise RuntimeError(
